...
# IMPORTING MODEL
diabates_model=pickle.load(open('D:/COLLEGE STUDIES/SEM-5/DE/Project/saved_models/diabetes_model.sav','rb'))
...
with st.sidebar:
    selected=option_menu('Multiple Disease Prediction System using ML',
                         ['Diabates Prediction','Heart Disease Prediction','Parkinsons Prediction'],
...
if (selected=='Diabates Prediction'):
    ...
    with col1:
        DiabatesPedigreeFunction=st.text_input('Diabates Pedigree Function value')
    with col2:
    ...
    if st.button('Diabates Test Result'):
        diab_prediction = diabates_model.predict([[Pregnancies,Glucose,BloodPressure,SkinThickness,Insulin,BMI,DiabatesPedigreeFunction,Age]])
